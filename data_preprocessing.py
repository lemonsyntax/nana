...
class DataPreprocessor:
    ...
    def preprocess_data(self, df: pd.DataFrame, target_column: str = 'Exam_Score', 
        ...
        df['Extracurricular_Activities'] = df['Extracurricular_Activities'].replace({'Yes': 1, 'No': 0, 'yes': 1, 'no': 0}).fillna(0).astype(int)
       
        # Map Parental_Education_Level
        edu_map = {'High School': 1, 'College': 2, 'Undergraduate': 2, 'Graduate': 3, 'Postgraduate': 4, 1: 1, 2: 2, 3: 3, 4: 4}
        df['Parental_Education_Level'] = df['Parental_Education_Level'].replace(edu_map).fillna(1).astype(int)
    ...
    def transform_new_data(self, df: pd.DataFrame) -> np.ndarray:
        ...
        # Fill missing
        df = df.fillna(df.mean(numeric_only=True))
        return np.array(df[self.feature_columns].values)
